...
export interface Settings { volume: number; sensitivity: number; quality: Quality; difficulty: Difficulty; opponent: OpponentStyle; trajectory: boolean; impact: boolean; landing: boolean; headMotion: boolean; controls: ControlMode; guides: boolean; minimal: boolean }
export const defaultSettings: Settings = { volume: 0.55, sensitivity: 1, quality: 'balanced', difficulty: 'casual', opponent: 'steady', controls: 'assisted', guides: true, minimal: false, trajectory: true, impact: true, landing: true, headMotion: true };
export function validateSettings(saved: unknown): Settings {
  ...
  for (const key of ['guides', 'minimal', 'trajectory', 'impact', 'landing', 'headMotion'] as const) if (typeof s[key] === 'boolean') result[key] = s[key];
  return result;
}
...
const freshSession = () => ({
  ...
  feedback: '', lastLanding: null as 'Target' | 'In' | 'Out' | null, nextFeed: 0, courtFade: 0, rallies: 0, bestRally: 0, swingReady: false, reachReady: false, contactPulse: 0, smashReady: false, selectedShot: 'rally' as AssistedShot, timedContact: false, swingPower: 1, flail: 0, shuttleSeen: true, swingMissed: false, opponentShot: '' as string, replyPulse: 0, winners: 0, smashWinners: 0, touchWinners: 0, topSpeed: 0,
