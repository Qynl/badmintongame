...
export class OpponentAI {
  ...
  reset() { this.target.copy(this.position); this.velocity.set(0, 0, 0); this.reaction = 0; this.lastFlight = -1; this.mayHit = true; this.leavingOut = false; this.predictionTimer = 0; this.pressure = 0; this.returns = 0; }
  step(dt: number, shuttle: ShuttlecockPhysics, player: Vector3, difficulty: Difficulty, hits: number, friendly = false, relaxed = false): boolean {
    ...
    if (incoming && this.lastFlight !== hits) {
      ...
      const paceError = MathUtils.clamp((shuttle.velocity.length() - 30) / 170, 0, 0.24);
      this.mayHit = Math.random() > level.miss + paceError; this.pressure = 0; this.readThisFlight = false; this.leavingOut = false;
    ...
    if (incoming && this.reaction === 0 && this.predictionTimer <= 0) {
      ...
      if (prediction.reachable && !this.leavingOut) {
        ...
        const required = this.position.distanceTo(this.target) / level.speed;
        this.pressure = Math.max(this.pressure, MathUtils.clamp(required / Math.max(0.15, prediction.time) - 0.6, 0, 1));
      }
