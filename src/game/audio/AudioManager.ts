...
export class AudioManager {
  ...
  sound(kind: 'hit' | 'step' | 'squeak' | 'net' | 'land' | 'flight', power: number, position: Vector3) {
    const ctx = this.context; if (!ctx || !this.master || ctx.state !== 'running') return;
    const t = ctx.currentTime, duration = kind === 'hit' ? 0.13 : kind === 'squeak' ? 0.15 : kind === 'flight' ? 0.1 : 0.09;
    const source = ctx.createBufferSource(); source.buffer = this.noise(duration);
    const filter = ctx.createBiquadFilter(); filter.type = 'bandpass'; filter.frequency.value = kind === 'hit' ? 1700 + power * 15 : kind === 'squeak' ? 2400 : kind === 'flight' ? 4300 : 260;
    filter.Q.value = kind === 'squeak' ? 16 : 0.7;
    const gain = ctx.createGain(); const volume = kind === 'hit' ? Math.min(0.8, 0.16 + power / 65) : kind === 'flight' ? 0.025 : kind === 'net' ? 0.07 : 0.16;
