...
export function GameHUD() {
  ...
  return <div className="game-hud"><CourtFade/>
    ...
    {phase === 'playing' && locked && <><SwingCoach/><span className="reticle"/>{!active && <div className="rally-message"><span>{message}</span><p>{mode === 'match' ? server === 0 ? <>{assisted ? <><kbd>CLICK</kbd> Serve <span>·</span> <kbd>E</kbd> also works</> : <><kbd>E</kbd> Release shuttle <span>·</span> Hold click & move to swing</>}</> : 'Take your receiving position' : <NextFeed/>}</p></div>}<div className="hud-bottom"><span><kbd>W A S D</kbd> MOVE <i/> <Mouse size={14}/> LOOK <i/>{assisted ? <><kbd>CLICK / HOLD</kbd> SWING</> : <><kbd>HOLD LMB</kbd> + MOVE TO SWING</>}</span><span>{mode !== 'match' && <><kbd>E</kbd> NEW FEED <i/></>}{rally > 0 && `${rally} CONTACT${rally === 1 ? '' : 'S'} · `}<kbd>ESC</kbd> PAUSE</span></div>{mode !== 'match' && <PracticeMetrics/>}</>}
    {phase === 'playing' && !locked && <div className="focus-overlay"><div><Mouse size={29}/><h2>Your place is on the court.</h2><p>{assisted ? <>Move with WASD. Look with your mouse. Click to hit.<br/>Hold click for extra help timing the return.</> : <>Hold click and move your mouse to swing the racket.<br/>Exact contact matters in Simulation mode.</>}<br/>Press Escape whenever you need a breather.</p><button className="primary-button" onClick={focusGame}>Ready when you are <ArrowUpRight size={21}/></button><button className="text-button" onClick={leaveGame}><ArrowLeft size={15}/> Back to the club</button></div></div>}
