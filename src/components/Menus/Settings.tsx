...
export function Settings({ onClose }: { onClose: () => void }) {
  ...
  return <Modal title="Make it your game." eyebrow="PREFERENCES" onClose={onClose}>
    <ControlChoice/>
    <section className="settings-section"><div className="setting-row"><div>Shuttle & swing cues<small>Clearer tracking and an in-reach indicator in Assisted mode</small></div><Toggle value={s.guides} onChange={() => set({ guides: !s.guides })} label="Shuttle and swing cues"/></div><div className="setting-row"><div>Minimal HUD<small>Hide the coach, metrics and key hints. Panels also fade while the shuttle is in the air.</small></div><Toggle value={s.minimal} onChange={() => set({ minimal: !s.minimal })} label="Minimal HUD"/></div></section>
    ...
    <section className="settings-section"><h3><SlidersHorizontal size={16}/> Practice tools</h3>{([['trajectory', 'Shuttle trail'], ['impact', 'Contact & swing metrics'], ['landing', 'Predicted landing marker']] as const).map(([key, label]) => <div className="setting-row" key={key}><span>{label}</span><Toggle value={s[key]} onChange={() => set({ [key]: !s[key] })} label={label}/></div>)}<p className="field-note">The landing marker and metrics stay out of competitive matches; the trail follows the shuttle in them too, which helps when the ball is high.</p></section>
