...
export function HowToPlay({ onClose }: { onClose: () => void }) {
  ...
  return <Modal title="Find your sweet spot." eyebrow="THE ESSENTIALS" onClose={onClose} wide>
    ...
    <div className="guide-grid">
      ...
      <article><Crosshair size={23}/><span>03 / MAKE CONTACT</span><h3>{assisted ? 'Just click to connect.' : 'Your mouse is your swing.'}</h3><p>{assisted ? 'Left-click for a deep rally return. Right-click for a soft drop, or a net shot up close. Press F for a smash when the high-ball cue lights up. Hold a control for timing help; tap during the window for extra attacking pace. No mouse flick required.' : 'Hold the left mouse button and move your mouse to guide your racket. Up raises it, down lowers it. The exact string bed must meet the shuttle; your motion determines its response.'}</p></article>
      <article><ScanLine size={23}/><span>04 / START A RALLY</span><h3>Serve. Read. Recover.</h3><p>{assisted ? <>Click to serve, or press <kbd>E</kbd>. Push the opponent back with a clear, draw them forward with a drop, then smash a short high reply. A low-contact F becomes a defensive return, not a fake smash. Space jumps for extra reach.</> : <>Press <kbd>E</kbd> to release your serve, then swing through the shuttle below 1.15 m into the diagonal service box.</>} Practice and training feed automatically; <kbd>E</kbd> restarts a feed. In training, use the requested shot and land in the highlighted zone. <kbd>Esc</kbd> pauses.</p></article>
    </div>
