...
import { styleList, styles } from '../../game/ai/Styles';
...
export function Setup({ mode, onClose }: { mode: Mode; onClose: () => void }) {
  const format = useGameStore((s) => s.matchFormat), setFormat = useGameStore((s) => s.setMatchFormat);
  ...
  return <Modal title={mode === 'match' ? 'Meet you at the net.' : mode === 'practice' ? 'Make yourself at home.' : 'One shot. A little better.'} eyebrow={mode === 'match' ? 'SINGLES / QUICK MATCH' : mode === 'practice' ? 'FREE PRACTICE' : 'SHOT TRAINING'} onClose={onClose}>
    <div className="setup-summary">{mode === 'match' ? <Trophy size={21}/> : mode === 'practice' ? <InfinityIcon size={23}/> : <Target size={22}/>}<div><strong>{mode === 'match' ? 'The classic. Just you and the game.' : mode === 'practice' ? 'Build a run. Beat your best.' : 'Read the feed. Refine your contact.'}</strong><p>{mode === 'match' ? format === 'duel' ? '7 points · Win by 2 · Cap at 11 · One decisive game' : '21 points · Best of 3 games · Rally scoring' : mode === 'practice' ? 'Rally Run challenges · Shot-chain bonuses · Saved best score' : 'Target zones · Landing feedback · Accuracy & streaks'}</p></div></div>
    {mode === 'match' && <div className="match-formats" role="group" aria-label="Match length">
      <button aria-pressed={format === 'duel'} onClick={() => setFormat('duel')}><Clock3 size={17}/><span><strong>Quick duel</strong><small>FIRST TO 7 · ONE MORE GAME</small></span>{format === 'duel' && <Check size={15}/>}</button>
      <button aria-pressed={format === 'classic'} onClick={() => setFormat('classic')}><Trophy size={17}/><span><strong>Club match</strong><small>21 POINTS · BEST OF THREE</small></span>{format === 'classic' && <Check size={15}/>}</button>
    </div>}
    ...
    {mode !== 'training' ? <><div className="section-label">YOUR OPPONENT</div><div className="difficulty-options">{levels.map((level) => <button key={level.id} className={`difficulty-option ${difficulty === level.id ? 'selected' : ''}`} onClick={() => set({ difficulty: level.id })}><span className="level-bars">{[1, 2, 3].map((bar) => <i key={bar} className={bar <= level.bars ? 'lit' : ''}/>)}</span><span><strong>{level.name}</strong><small>{level.description}</small></span><span className="radio">{difficulty === level.id && <Check size={12}/>}</span></button>)}</div>
      <div className="section-label">HOW THEY PLAY</div><div className="style-options" role="group" aria-label="Opponent style">{styleList.map((id) => <button key={id} className={`style-option ${opponent === id ? 'selected' : ''}`} aria-pressed={opponent === id} onClick={() => set({ opponent: id })}><span><strong>{styles[id].name}</strong><small>{styles[id].blurb}</small></span>{opponent === id && <Check size={13}/>}</button>)}</div></> : <><div className="section-label">FOCUS ON A SHOT</div><div className="shot-options">{(['Clear', 'Drop', 'Smash', 'Net shot'] as TrainingShot[]).map((item) => <button className={shot === item ? 'selected' : ''} key={item} onClick={() => setShot(item)}>{item}{shot === item && <Check size={14}/>}</button>)}</div><p className="field-note">Match the shot and land inside the highlighted zone to score a success. {shot === 'Smash' ? 'In Assisted mode, press F on the high-ball cue. Time a fresh press for extra pace; Space jumps for reach.' : shot === 'Clear' ? 'Lift the face and follow through upward. Aim deep into the back court.' : shot === 'Drop' ? 'In Assisted mode, right-click for a soft drop. Aim into the highlighted front-court zone.' : 'Step near the service line. In Assisted mode, right-click for a delicate net shot.'}</p></>}
