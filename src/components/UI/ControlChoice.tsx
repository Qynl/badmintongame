...
export function ControlChoice({ compact = false }: { compact?: boolean }) {
  ...
  return <section className={`control-choice ${compact ? 'compact-choice' : ''}`} aria-label="Swing controls"><div className="section-label">HOW YOU WANT TO PLAY</div><div className="control-options">
    <button className={controls === 'assisted' ? 'selected' : ''} aria-pressed={controls === 'assisted'} onClick={() => set({ controls: 'assisted' })}><Hand size={18}/><span><strong>Assisted <em>RECOMMENDED</em></strong><small>Easy contact. Three shots. You pick the landing spot.</small></span>{controls === 'assisted' && <Check size={15}/>}</button>
  ...
  </div><p>{controls === 'assisted' ? 'Look at the spot you want and click: the marker on the court is where the shuttle lands. Right-click drops, F smashes a high ball. Hold a control for timing help, or tap as the window opens for a cleaner attack.' : 'Hold click and move the mouse to position and accelerate the racket. Every contact is physical and unassisted.'}</p></section>;
}
